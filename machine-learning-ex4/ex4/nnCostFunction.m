...
hypothesis = sigmoid(Theta2*layer1');



for i=1:m
    for k = 1:num_labels
        J = J+(1/m)*((-(y(i,:)==k))*log(hypothesis(k,i))-(1-(y(i,:)==k))*log(1-hypothesis(k,i)));   
    end
end
...
for j=1:hidden_layer_size
    ...
    for k=1:input_layer_size
        regularizationCost =regularizationCost+tmpTheta1(j,k)^2;
    end
end

for j=1:num_labels
    for k=1:hidden_layer_size
        regularizationCost = regularizationCost+ tmpTheta2(j,k)^2;
    end
end

regularizationCost = regularizationCost*lambda/(2*m);
